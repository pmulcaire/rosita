...
class ElmoTokenEmbedder(TokenEmbedder):
    ...
    def __init__(self,
        ...
        """
        Import has to be here because of circular dependency between modules/token_embedders/ and modules/elmo.py.
        elmo.py relies on token_embedders/embedding.py, but to load embedding.py, we must also load this file 
        (elmo_token_embedder.py), which imports elmo.py. Not a true circular dependency, just determined by the 
        package structure.
        """
        ...
        print("\nCheck to see where this (elmo_token_embedder.py) is created. When creating Elmo (and _ElmoBilm) need to have a way to indicate that it uses words/chars/words+chars\n\n")
