...
from allennlp.data.token_indexers.elmo_indexer import PreloadedElmoCharacterMapper, ELMoTokenCharactersIndexer
from allennlp.data.token_indexers.elmo_token_indexer import PreloadedElmoTokenMapper, ELMoTokenIndexer
...
class _ElmoCharacterEncoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.char_mapper = PreloadedElmoCharacterMapper(self._options['char_cnn']['char_map'])
        ...
        self._end_of_sentence_characters = torch.from_numpy(
                numpy.array(self.char_mapper.end_of_sentence_characters) + 1
        )
...
class _ElmoBiLm(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if vocab_to_cache:
            ...
            # This sets 3 attributes, _cache_embedding, _bos_embedding and _eos_embedding.
    ...
    def forward(self,  # pylint: disable=arguments-differ
        ...
        """
        Compute char-based embedding (try cache first, then use CNN)
        """
        if self._cache_embedding is not None:
            try:
                mask_without_bos_eos = (word_inputs > 0).long()
                # The character cnn part is cached - just look it up.
                char_embedded_inputs = self._cache_embedding(word_inputs) # type: ignore
                # shape (batch_size, timesteps + 2, embedding_dim)
                ctype_representation, maskc = add_sentence_boundary_token_ids(
                ...
                char_token_embedding = self._char_token_embedder(inputs)
    ...
    def create_cached_cnn_embeddings(self, tokens: List[str]) -> None:
        ...
        _cache_embedding : ``torch.Tensor``
            The cached character-based word embedding for each word in the tokens passed to this method.
        _bos_embedding : ``torch.Tensor``
            The character-based embedding for the BOS token.
        _eos_embedding : ``torch.Tensor``
            The character-based embedding for the EOS token.
        ...
        tokens = [self.char_mapper.bos_token, self.char_mapper.eos_token] + tokens 
        timesteps = 32
        batch_size = 32
        ...
        for batch in lazy_groups_of(chunked_tokens, batch_size):
            ...
            output = self._char_token_embedder(batched_tensor)
            char_token_embedding = output["token_embedding"]
            mask = output["mask"]
            char_token_embedding, _ = remove_sentence_boundaries(char_token_embedding, mask)
            all_embeddings.append(char_token_embedding.view(-1, char_token_embedding.size(-1)))
        ...
        self._cache_embedding = Embedding(vocab_size, # type: ignore
                                         embedding_dim,
