...
@Model.register("biaffine_parser_adv_zero")
class BiaffineDependencyParserAdvZero(Model):
    """
    ...
    def __init__(self,
        ...
        super(BiaffineDependencyParserAdvZero, self).__init__(vocab, regularizer)
        ...
        num_langs = self.vocab.get_vocab_size("lang_ids")
        #self.tag_projection_layer = TimeDistributed(Linear(encoder_dim, num_langs))
        self.tag_projection_layer = TimeDistributed(Linear(arc_representation_dim, num_langs))
    ...
    def forward(self,  # type: ignore
        ...
        if head_indices is not None and head_tags is not None:
            ...
            arc_nll, tag_nll = self._construct_loss(head_tag_representation=head_tag_representation,
                                                    ...
                                                    head_tags=head_tags,
                                                    mask=mask, langid=langid)
            loss = arc_nll + tag_nll
            source_lang = (1 - langid==0).view([-1, 1]).long()
            evaluation_mask = self._get_mask_for_eval(mask[:, 1:], pos_tags)
            evaluation_mask = evaluation_mask*source_lang
            ...
            arc_nll, tag_nll = self._construct_loss(head_tag_representation=head_tag_representation,
                                                    ...
                                                    mask=mask, langid=langid)
            loss = arc_nll + tag_nll
        langid = langid.unsqueeze(1).repeat(1, sequence_length+1)
        #adv_rep = self.tag_projection_layer(grad_reverse(encoded_text))
        adv_rep = self.tag_projection_layer(grad_reverse(head_arc_representation))
        adv_loss = self._construct_lang_loss(adv_rep, langid, mask)*self.adv_coef
        loss += adv_loss
    ...
    def _construct_loss(self,
                        ...
                        mask: torch.Tensor,
                        langid: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        """
        ## adversarial training. Ignore target language (langid 0) annotations.
        source_lang = (1 - langid==0).view([-1, 1]).long()
        float_mask = mask.float()
        ...
        valid_positions = (mask*source_lang).sum() - source_lang.sum()

        arc_nll = (-arc_loss*source_lang.float()).sum() / valid_positions.float()
        tag_nll = (-tag_loss*source_lang.float()).sum() / valid_positions.float()
        return arc_nll, tag_nll

    def _construct_lang_loss(self,
                        logits: torch.Tensor,
                        tags: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        loss = sequence_cross_entropy_with_logits(logits, tags, mask, average = "token")
        return loss
